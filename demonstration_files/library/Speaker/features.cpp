...
// Description du programme : Le fichier features sert à implémenter différentes melodies dans le piezo.
...
#define WHOLE_NOTE_duree_MS 80750
#define NOTE_B0 31
#define NOTE_C1 33
#define NOTE_CS1 35
#define NOTE_D1 37
...
void Speaker::wakeUp()
{
    ...
    uint32_t numNotes = sizeof(melodie) / sizeof(melodie[0]) / 2;

    for (uint32_t i = 0; i < numNotes; i++)
    {
        uint8_t note = melodie[i * 2];
        uint8_t duree = melodie[i * 2 + 1];

        setSoundCustom(note);
        for (int y = 0; y < duree*10; y++)
        {
            _delay_ms(1);
        }
    }
